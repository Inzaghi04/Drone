...
void setMotorSpeed(ledc_channel_t channel, float input) {
    // Map giá trị throttle từ Web (0-255) → 1000-2000µs
    ...
    uint32_t duty = (uint32_t)(
        (mapped_input - 1000) * (PWM_MAX_DUTY - PWM_MIN_DUTY) / 1000 + PWM_MIN_DUTY
    );
...
static esp_err_t control_post_handler(httpd_req_t *req) {
    ...
    setMotorSpeed(LEDC_CHANNEL_4, web_control_data.throttle);
...
void app_main() {
    ...
    configureMotor(ESC_MOTOR_1, LEDC_CHANNEL_1);
    configureMotor(ESC_MOTOR_2, LEDC_CHANNEL_2);
    configureMotor(ESC_MOTOR_3, LEDC_CHANNEL_3);
    configureMotor(ESC_MOTOR_4, LEDC_CHANNEL_4);

    ESP_LOGI(TAG, "Bắt đầu Calibrate ESC...");
    
    // Bước 1: Gửi PWM MAX (2000µs) trong 5 giây
    setMotorSpeed(LEDC_CHANNEL_1, 255); // 255 → mapped thành 2000µs
    setMotorSpeed(LEDC_CHANNEL_2, 255);
    setMotorSpeed(LEDC_CHANNEL_3, 255);
    setMotorSpeed(LEDC_CHANNEL_4, 255);
    vTaskDelay(pdMS_TO_TICKS(5000)); // Đợi 5 giây

    // Bước 2: Gửi PWM MIN (1000µs) để hoàn tất
    setMotorSpeed(LEDC_CHANNEL_1, 0); // 0 → mapped thành 1000µs
    setMotorSpeed(LEDC_CHANNEL_2, 0);
    setMotorSpeed(LEDC_CHANNEL_3, 0);
    setMotorSpeed(LEDC_CHANNEL_4, 0);
